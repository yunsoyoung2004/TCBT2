...
const APPROVED_TEXT: Record<string, string> = {
  ...
  // §5.2 [신규 N-3, P1→P0]: CCGH previously reused the CCPH color list with
  // no anchor meanings at all, while its own validation
  // (exact_scale_anchors) already expected six real anchors -- and told the
  // participant "the same 0-5 color scale" as problems, which is false (only
  // the colors repeat; the six meanings are goal-specific). Full anchor text
  // restored verbatim from the S02 manual Annex CCGH table (§Appendix A-5).
  "tbct-s02-n08-p02-six-anchor-goal-scale": "Now let's rate how difficult or distressing each goal feels to pursue right now. This scale uses the same colors as before, but the meanings are different: 0 Light blue — easy and comfortable to achieve, or already achieved; 1 Dark blue — not so easy or comfortable to achieve; 2 Light green — difficult or uncomfortable to achieve; 3 Dark green — very difficult or uncomfortable to achieve; 4 Yellow — achieving it is distressing and/or really hard; 5 Red — achieving it is so distressing that you cannot imagine yourself trying.",
...
export const koreanText: Record<string, string> = {
  ...
  // §5.2, §Appendix A-5 (CCGH, verbatim from S02 manual Annex, KO).
  "tbct-s02-n08-p02-six-anchor-goal-scale": "이제 각 목표를 지금 추구하는 것이 얼마나 어렵거나 힘들게 느껴지는지 평가해 볼게요. 색상은 앞서와 같지만 의미는 다릅니다. 0 연한 파란색 — 쉽고 편안하게 달성할 수 있거나 이미 달성함; 1 진한 파란색 — 그리 쉽거나 편안하지는 않음; 2 연한 초록색 — 달성이 어렵거나 불편함; 3 진한 초록색 — 달성이 매우 어렵거나 불편함; 4 노란색 — 달성하는 것이 괴롭고 정말 힘듦; 5 빨간색 — 너무 괴로워서 시도하는 자신을 상상하기 어려움.",
